...
if (!function_exists('trx_utils_support_menuitems_post_type')) {
	...
	function trx_utils_support_menuitems_post_type($name, $args=false) {
		if ($name=='menuitems') {
			if ($args===false) {
				$args = array(
					'label'               => esc_html__( 'Menu items', 'ancora-utils' ),
					'description'         => esc_html__( 'Menu Items Description', 'ancora-utils' ),
					'labels'              => array(
						'name'                => esc_html__( 'Menu items', 'ancora-utils' ),
						'singular_name'       => esc_html__( 'Menu item', 'ancora-utils' ),
						'menu_name'           => esc_html__( 'Menu items', 'ancora-utils' ),
						'parent_item_colon'   => esc_html__( 'Parent Item:', 'ancora-utils' ),
						'all_items'           => esc_html__( 'All menu items', 'ancora-utils' ),
						'view_item'           => esc_html__( 'View Item', 'ancora-utils' ),
						'add_new_item'        => esc_html__( 'Add New menu items', 'ancora-utils' ),
		...
		if ($name=='menuitems_group') {
			if ($args===false) {
				$args = array(
					...
					'hierarchical'      => true,
					'labels'            => array(
						'name'              => esc_html__( 'Menu Items Group', 'ancora-utils' ),
						...
						'menu_name'         => esc_html__( 'Menu items Group', 'ancora-utils' ),
					),
					'show_ui'           => true,
